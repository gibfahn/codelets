...
fn calc_tile(left: Tile, centre: Tile, right: Tile) -> Tile {
    use Tile::*;
    if left == Trap && centre == Trap && right != Trap ||
       centre == Trap && right == Trap && left != Trap ||
       left == Trap && centre != Trap && right != Trap ||
       right == Trap && centre != Trap && left != Trap
    {
        Trap
    } else {
        Safe
...
#[derive(Debug)]
struct Floor { v: Vec<Vec<Tile>> }

impl Floor {
    fn new(s: &str, rows: usize) -> Self {
        //println!("\nString: {}", s);
        let lines = s.lines().map(|x| x.trim());
        let columns = s.lines().map(|x| x.trim()).take(1).collect::<String>().len();
        ...
        for (row, line) in lines.enumerate() {
            if line.len() != columns {
                panic!(format!("Line {} length is wrong, length: {}, columns: {}", row, line.len(), columns));
            }
    ...
    fn get(&self, row: usize, col: usize) -> Option<Tile> {
        self.v.get(row).and_then(|r| r.get(col)).cloned()
    }
        ...
        for row in 0..self.v.len() {
            for col in 0..self.v[0].len() {
                if self.get(row, col) == Some(Unknown) {
                    if row == 0 { panic!("First row must be filled"); }
                    let left = if col == 0 { Safe } else {
                        self.get(row - 1, col - 1).unwrap() };
                    let centre = self.get(row - 1, col).unwrap();
                    let right = if col == self.v[0].len() - 1 { Safe } else {
                        self.get(row - 1, col + 1).unwrap()
                    };
                    self.v[row][col] = calc_tile(left, centre, right);
                }
            }
        }
    ...
    fn count_safe(&self) -> usize {
        self.v.iter().fold(0, |acc, row| acc + row.iter().filter(|x| **x == Tile::Safe).count())
    }
}
